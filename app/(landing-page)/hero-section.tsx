...
const tabs = [
  {
    ...
    name: "Create TrustID",
    description: "Get your TrustID on the blockchain",
    more: (
      <div className="text-red-600 flex items-center">
        Learn more
        <PiArrowRight className="ml-1 text-sm" />
      </div>
    ),
    images: "/assets/PlantDoodle.svg",
  },
  {
    icon: (
...
const HeroSection = () => {
  ...
  return (
    <div className="md:items-center flex flex-col">
      <div
        className="font-medium 2xl:w-1/3 md:w-2/3 xl:w-1/2 lg:px-0
             px-8 text-5xl xl:text-6xl flex justify-center xl:font-medium
             xl:pt-14 text-center pt-6"
      >
        Your Digital Identity, Reinvented{" "}
      </div>
      <p className="text-2xl pt-4 text-center w-2/3 mx-auto">
        <span className="text-sky-500 font-bold">TrustID</span>
      </p>
        ...
        <Link href="/">
          <Button className="py-1 ">
            <div className="flex items-center justify-center">
              <div className="text-lg">Create TrustID</div>
              <div>
      ...
      {isSmallScreen ? (
        <div
          className="md:grid-cols-1 grid sm:grid-cols-2  place-content-center 
        place-items-center mx-auto space-x-0  xl:space-x-4 items-center justify-between 
        hover:cursor-pointer gap-4 w-4/5 xl:w-3/4 2xl:w-[55%]"
        >
          {tabs.map((tab) => (
            <motion.div
              key={tab.name}
              className={`xl:flex justify-center space-x-4 xl:pt-4 sm:my-10
              xl:my-0 w-60 h-36
             ${
               activeTab === tab
